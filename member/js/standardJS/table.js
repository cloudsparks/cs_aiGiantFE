...
function buildTable(data, tableId, divId, thArray, btnArray, message, tableNo) {
    ...
    $('#'+divId).prev().html('').hide();

    if(!data){
        if(!message){
            message = `${translations["M00019"][language]}`;
        }

        $('#'+divId).prev().html(`
            ${buildTableHead(thArray)}
            <div class="mt-4 mb-3"><img src="images/project/noResultFound.png" width="80px"/></div>
            <div>${message}</div>
        `).show();
        return;
    }
...
function pagination(pagerId, pageNumber, totalPage, totalRecord, numRecord) {
    ...
    if(pageNumber > 1) {
        var paginateFirst = translations["M01319"][language]; //FIRST
    ...
    if(pageNumber < totalPage) {
        var paginateLast = translations["M01320"][language]; //LAST
