...
<script>
    ...
    $(document).ready(function() {
        ...
        if(!id) {
            ...
            showMessage(message, '', 'Error', 'exclamation-triangle', 'banner.php');
            return;
        }

        formData  = {command : 'getBanner', id : id};
    ...
    function addImage(){
        ...
        function uploadVideoSuccess() {
            showMessage('Successful edit banner', 'success', 'Congratulations', 'upload', 'banner.php');
        }
     function loadDefaultData(data, message) {

        $('#subject').val(data.banner.subject);
        ...
        else if(data.banner.status == "Inactive")
            $("#inActive").prop("checked", true);
        else if(data.banner.status == "Deleted")
        ...
        $.each(data.bannerDetail, function(i,obj){
            selectedLang.push(i);
